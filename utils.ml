 ...
 *)
let ( @=> ) (o:operator->operator) (o':operator) = o o'
let ( @==> ) (o:operator->(operator*operator)) (o':operator) = o o'
...
let string_of_op_result r = match r with
    ...
    | Int32 i -> sprintf "%lu" i (* Int32.to_string i *)
...
[%%cstruct
type icmp = {
    itype: uint8_t; (* itype because type is a keyword *)
    code: uint8_t;
    checksum: uint16_t;
} [@@big_endian]]
...
let parse_ipv4 ip m = 
    ...
    (Tuple.add "ipv4.flags" (Int ((get_ipv4_off ip) lsr 13)))
...
let parse_tcp tcp m = 
    ...
    (Tuple.add "l4.ack" (Int32 (get_tcp_acknum tcp))) |>
    (Tuple.add "l4.window" (Int (get_tcp_window tcp))) |>
    (Tuple.add "l4.icmp_type" (Int 0)) |>
    (Tuple.add "l4.icmp_code" (Int 0))
...
let parse_udp udp m =
    ...
    (Tuple.add "l4.ack" (Int32 0l)) |>
    (Tuple.add "l4.window" (Int 0)) |>
    (Tuple.add "l4.icmp_type" (Int 0)) |>
    (Tuple.add "l4.icmp_code" (Int 0))

let parse_icmp icmp m =
    m |>
    ...
    (Tuple.add "l4.ack" (Int32 0l)) |>
    (Tuple.add "l4.window" (Int 0)) |>
    (Tuple.add "l4.icmp_type" (Int (get_icmp_itype icmp))) |>
    (Tuple.add "l4.icmp_code" (Int (get_icmp_code icmp)))
...
let parse_pkt network h hdr p = 
    ...
    try
        ...
        let res = (
            ...
            | 1 -> parse_icmp (Cstruct.shift p offset) res
